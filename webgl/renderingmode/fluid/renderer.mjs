...
const progA = createProgram(gl, vertexShaderSourceCode, fragmentShaderBufferSourceCode);
...
renderer.renderGame = function (game, time, deltaTime)
{
...

    {
        ...
        if (ball != null)
        {
            ...
            gl.vertexAttribPointer(loc,2,gl.FLOAT,false,ball.x,ball.y);
        ...
        if (player != null)
        {
            ...
            gl.bindBuffer(gl.ARRAY_BUFFER, quad);
            gl.enableVertexAttribArray(loc);
            gl.vertexAttribPointer(loc,2,gl.FLOAT,false,player.x,player.y);
            gl.uniform3f(gl.getUniformLocation(progA,"iResolution"),canvas.width,canvas.height,1);
            gl.uniform1f(gl.getUniformLocation(progA,"iTime"),time*0.001);
            gl.uniform1f(gl.getUniformLocation(progA,"iTimeDelta"),dt);
            gl.activeTexture(gl.TEXTURE0);
        ...
        rects.forEach(rect =>
        {
            ...
            gl.bindBuffer(gl.ARRAY_BUFFER, quad);
            gl.enableVertexAttribArray(loc);
            gl.vertexAttribPointer(loc,2,gl.FLOAT,false,rect.x,rect.y);
            gl.uniform3f(gl.getUniformLocation(progA,"iResolution"),canvas.width,canvas.height,1);
            gl.uniform1f(gl.getUniformLocation(progA,"iTime"),time*0.001);
            gl.uniform1f(gl.getUniformLocation(progA,"iTimeDelta"),dt);
            gl.activeTexture(gl.TEXTURE0);
