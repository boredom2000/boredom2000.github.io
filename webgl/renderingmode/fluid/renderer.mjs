export let renderer = {};
const vertexBufferShaderSourceCode = await loadShaderSource('/renderingmode/fluid/vertex.glsl');
const fragmentShaderBufferSourceCode = await loadShaderSource('/renderingmode/fluid/fragment.glsl');
...
gl.uniform1i(gl.getUniformLocation(progA, "uNoiseTexture"), 0);

function updatePlayArea()
{
    fbo1 = createFBO(gl, canvas.width, canvas.height);
    fbo2 = createFBO(gl, canvas.width, canvas.height);
    noiseTex = makeNoise(gl, 256);
...
renderer.renderGame = function (game, time, deltaTime)
{
    ...
    gl.uniform1i(gl.getUniformLocation(progA,"uSourceTexture"),1);
...

    {
        ...
        rects.forEach(rect =>
        {
          ...
          if (true)
          {
            ...
            gl.uniform1i(gl.getUniformLocation(progA,"uSourceTexture"),0);
            gl.drawArrays(gl.TRIANGLES, 0, 6);
